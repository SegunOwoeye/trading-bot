...
def Buy_or_sell_decision():
    ...
    if MA_cross_check() == "CROSSED": # Checks to see if moving averages have cross
        if str(stating_five_unit_state()) == "POSITIVE" and str(stating_ten_unit_state()) == "POSITIVE":
            print("Upward Market Trend")
            if five_unit_MA()>ten_unit_MA():#Buyers Market
                print("Checks Complete, Purchasing BNB")
            
                BNB_Buy()

            else: # Wait for 50 minutes to see if it's time to all checks give green light, if not, won't do anything
                print("Conditions not yet met, waiting")
                time.sleep(60*60-60*60/6)
                if str(stating_five_unit_state()) == "POSITIVE" and str(stating_ten_unit_state()) == "POSITIVE":
                    print("Checks Complete, Purchasing BNB")
                    BNB_Buy()

                elif five_unit_MA()>ten_unit_MA(): #Buyers Market
                    print("Purchasing BNB")
                    BNB_Buy()
            ...
            print('Downward Market Trend')
        
            if ten_unit_MA()>five_unit_MA(): # Sellers market
                print("Time to Sell") 
                Sell_BNB_to_GBP()
